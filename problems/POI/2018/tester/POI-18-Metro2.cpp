...
void dfs_mark(int v, int p, int r) {
    ...
    for (int to : adj[v]) {
        if (to != p) {
            // // dbg(v, to);
            dfs_mark(to, v, r);
            close[v] = min(close[v], close[to] + 1);
            if (marked[to]) marked[v] = true;
        }
    }
    for (int to : adj[v]) {
        if (to != p) {
            if (dp[to] + 1 > close[v]) dp[v] = max(dp[v], dp[to] + 1);
        }
    }
    if (dp[v] == r || (v == 0 && dp[v] > 0)) {
        ++placed;
        dbg("MARKING", v + 1);
        marked[v] = true;
        init[v] = true;
...
void tri(int r, bool stor) {
    ans = 0;
    placed = 0;
    FOR(i, n) marked[i] = false, dp[i] = -1e9, cnt[i] = 0, close[i] = 0, init[i] = false;
    dfs_mark(root, -1, r);
    FOR(i, n) {
